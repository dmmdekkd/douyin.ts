...
export function uploadProcessFunctions (
  fileType: 'image' | 'video' | 'object',
  imageFormat?: ImageFormat,
): UploadProcessFunction[] {
  ...
  return [{
    ...
    input: {
      ...
      PolicyParams: imageFormat === 'gif'
        ? { 'policy-set': 'still', 'still-width': '480', 'still-height': '480' }
...
export class Uploader {
  ...
  private async credentials (configKey = 'public_image_config'): Promise<UploadCredentials> {
    ...
    const json = asRecord(JSON.parse(response.text))
  ...
  private async apply (
    ...
    const query = {
      ...
      FileType: fileType, IsInner: '1', NeedFallback: 'true', FileSize: String(fileSize), s: randomId(11),
    ...
    if (!nodes?.length && directStores?.length && directHosts?.length && fileType !== 'object') {
      ...
      return {
        storeUri: String(store['StoreUri'] ?? ''), authorization: String(store['Auth'] ?? ''),
        host: String(directHosts[0]), sessionKey: String(direct['SessionKey'] ?? ''),
      }
    }
    ...
    const address: UploadAddress = {
      storeUri: String(store['StoreUri'] ?? ''), authorization: String(store['Auth'] ?? ''),
      host: String(node['UploadHost'] ?? ''), sessionKey: String(node['SessionKey'] ?? ''),
      uploadHeaders: Object.fromEntries(
        Object.entries(asRecord(node['UploadHeader'])).map(([key, value]) => [key, String(value)]),
      ),
    }
    if (fileType === 'object') {
  ...
  private async putObject (address: UploadAddress, data: Uint8Array): Promise<void> {
    ...
    const result = asRecord(JSON.parse(response.text))
  ...
  private async initParts (address: UploadAddress): Promise<string> {
    ...
    const response = await this.http.request(`https://${address.host}/upload/v1/${address.storeUri}?phase=init`, {
      method: 'POST',
      headers: { ...this.storageHeaders(address.authorization), 'Content-Type': `multipart/form-data; boundary=${boundary}` },
      body: `--${boundary}--\r\n`,
    })
    const result = asRecord(JSON.parse(response.text))
    const uploadId = String(asRecord(result['data'])['uploadid'] ?? '')
    if (!response.ok || Number(result['code']) !== 2000 || !uploadId) throw new Error('TOS multipart init failed')
    return uploadId
  }
  ...
  private async transferPart (address: UploadAddress, uploadId: string, part: number, crc: string, data: Uint8Array): Promise<void> {
    ...
    const response = await this.http.request(`https://${address.host}/upload/v1/${address.storeUri}?${query}`, {
      method: 'POST',
      headers: { ...this.storageHeaders(address.authorization, crc), 'Content-Disposition': 'attachment; filename="undefined"' },
      body: new Uint8Array(data),
    })
    const result = asRecord(JSON.parse(response.text))
    if (!response.ok || Number(result['code']) !== 2000) throw new Error(`TOS part ${part} failed`)
  }
  ...
  private async finishParts (address: UploadAddress, uploadId: string, manifest: string): Promise<void> {
    ...
    const response = await this.http.request(`https://${address.host}/upload/v1/${address.storeUri}?${query}`, {
      method: 'POST',
      headers: { ...this.storageHeaders(address.authorization), 'Content-Type': 'text/plain;charset=UTF-8' },
      body: manifest,
    })
    const result = asRecord(JSON.parse(response.text))
    if (!response.ok || Number(result['code']) !== 2000) throw new Error('TOS multipart finish failed')
  }
  ...
  private async commit (
    ...
    const json = await this.signedVodJson('POST', {
      Action: 'CommitUploadInner', Version: '2020-11-19', SpaceName: credentials.spaceName,
    }, body, credentials)
    ...
    const committed: CommittedUpload = {
      uri: String(encryption['Uri'] ?? ''), secretKey: String(encryption['SecretKey'] ?? ''),
  ...
  private async signedVodJson (
    method: 'GET' | 'POST', query: Record<string, string>, body: Uint8Array | undefined,
    credentials: UploadCredentials,
  ...
  private storageHeaders (authorization: string, crc?: string): Record<string, string> {
    const headers: Record<string, string> = {
