...
export function aBogus (options: ABogusOpts): string {
  ...
  const userAgentLmStr = rc4Parse(
    String.fromCharCode(...[0.00390625, 1, keyVal]),
    options.userAgent,
  )
  const userAgentArr32 = sm3Digest(
    lmStrEncode(strToByteArr(userAgentLmStr), TABLES.s3),
  )
...
export function aBogusDesktop (options: DesktopABogusOpts): string {
  ...
  const uaHash = sm3Digest(
    lmStrEncode(
      garble(uaSbox(0), encodeUtf8ForSm3(options.userAgent)),
      TABLES.s3,
    ),
  )
  ...
  const order = [
    9, 18, 30, 35, 47, 4, 44, 19, 10, 23, 12, 40, 25, 42, 3, 22, 38, 21, 5,
    45, 1, 29, 6, 43, 33, 14, 36, 37, 2, 46, 15, 48, 31, 26, 16, 13, 8, 41,
    27, 17, 39, 20, 11, 0, 34, 7, 50, 51, 53, 54,
  ]
...
function xorRandomArr8Gen (): number[] {
  ...
  const n7 =
    (((((((randomVal >> 0) & 77) | 2) | 16) | 32) | 128) & 170) | 16
  const n8 =
    (((((((randomVal >> 0)) & 77) | 16) | 32) | 128) & 85) | 2
...
function sm3Digest (data: string | number[]): number[] {
  const bytes =
    typeof data === 'string' ? encodeUtf8ForSm3(data) : data
  const h = new Sm3()
  return h.sum(bytes)
}
